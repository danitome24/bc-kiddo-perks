...
contract StdCheatsTest is Test {
  ...
  function test_ParseJsonTxDetail() public view {
    ...
    string memory path =
      string.concat(root, "/test/fixtures/broadcast.log.json");
  ...
  function testFuzz_AssumeAddressIsNot(
    address addr
  ) external {
  ...
  function testFuzz_AssumeNotPrecompile(
    address addr
  ) external {
  ...
  function testFuzz_AssumeNotForgeAddress(
    address addr
  ) external pure {
  ...
  function test_DeployCodeTo() external {
    ...
    MockContractWithConstructorArgs ct =
      MockContractWithConstructorArgs(arbitraryAddress);
...
contract StdCheatsMock is StdCheats {
  function exposed_assumePayable(
    address addr
  ) external {
    assumePayable(addr);
  }

  function exposed_assumeNotPayable(
    address addr
  ) external {
...
contract StdCheatsForkTest is Test {
  ...
  function testFuzz_AssumeNotBlacklisted_USDC(
    address addr
  ) external view {
  ...
  function testFuzz_AssumeNotBlacklisted_USDT(
    address addr
  ) external view {
