...
abstract contract StdCheatsSafe {
  ...
  function assumeNotBlacklisted(
    ...
    (success, returnData) =
      token.staticcall(abi.encodeWithSelector(0xfe575a87, addr));
    vm.assume(!success || abi.decode(returnData, (bool)) == false);

    // 4-byte selector for `isBlackListed(address)`, used by USDT.
    (success, returnData) =
      token.staticcall(abi.encodeWithSelector(0xe47d6060, addr));
    vm.assume(!success || abi.decode(returnData, (bool)) == false);
  }
  ...
  function deployCode(
    ...
    require(
      addr != address(0), "StdCheats deployCode(string): Deployment failed."
    );
  }
  ...
  function _bytesToUint(
    ...
    require(
      b.length <= 32, "StdCheats _bytesToUint(bytes): Bytes length exceeds 32."
    );
...
abstract contract StdCheats is StdCheatsSafe {
  ...
  function deal(
    ...
    (, bytes memory balData) =
      token.staticcall(abi.encodeWithSelector(0x70a08231, to));
    ...
    if (adjust) {
      (, bytes memory totSupData) =
        token.staticcall(abi.encodeWithSelector(0x18160ddd));
  ...
  function dealERC1155(
    ...
    // get current balance
    (, bytes memory balData) =
      token.staticcall(abi.encodeWithSelector(0x00fdd58e, to, id));
    uint256 prevBal = abi.decode(balData, (uint256));

    // update balance
    stdstore.target(token).sig(0x00fdd58e).with_key(to).with_key(id)
      .checked_write(give);

    // update total supply
    if (adjust) {
      (, bytes memory totSupData) =
        token.staticcall(abi.encodeWithSelector(0xbd85b039, id));
      require(
        totSupData.length != 0,
  ...
  function dealERC721(address token, address to, uint256 id) internal virtual {
    ...
    require(
      successMinted, "StdCheats deal(address,address,uint,bool): id not minted."
    );
    ...
    (, bytes memory toBalData) =
      token.staticcall(abi.encodeWithSelector(0x70a08231, to));
    uint256 toPrevBal = abi.decode(toBalData, (uint256));

    // update balances
    stdstore.target(token).sig(0x70a08231).with_key(
      abi.decode(ownerData, (address))
  ...
  function deployCodeTo(
    ...
    (bool success, bytes memory runtimeBytecode) =
      where.call{ value: value }("");
    require(
      success,
