...
contract ERC2771Forwarder is EIP712, Nonces {
  ...
  event ExecutedForwardRequest(
    address indexed signer, uint256 nonce, bool success
  );
  ...
  error ERC2771ForwarderMismatchedValue(
    uint256 requestedValue, uint256 msgValue
  );
  ...
  function verify(
    ...
    (bool isTrustedForwarder, bool active, bool signerMatch,) =
      _validate(request);
  ...
  function _isTrustedByTarget(
    ...
    assembly {
      ...
      success :=
        staticcall(
          gas(), target, add(encodedParams, 0x20), mload(encodedParams), 0, 0x20
        )
