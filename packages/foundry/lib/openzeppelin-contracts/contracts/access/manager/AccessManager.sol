...
contract AccessManager is Context, Multicall, IAccessManager {
  ...
  function hasRole(
    ...
    if (roleId == PUBLIC_ROLE) {
      ...
      return
        (hasRoleSince != 0 && hasRoleSince <= Time.timestamp(), currentDelay);
    }
  }
  ...
  function _grantRole(
    ...
    if (newMember) {
      ...
      _roles[roleId].members[account] =
        Access({ since: since, delay: executionDelay.toDelay() });
    } else {
  ...
  function _setGrantDelay(uint64 roleId, uint32 newDelay) internal virtual {
    ...
    uint48 effect;
    (_roles[roleId].grantDelay, effect) =
      _roles[roleId].grantDelay.withUpdate(newDelay, minSetback());
  ...
  function _setTargetAdminDelay(
    ...
    uint48 effect;
    (_targets[target].adminDelay, effect) =
      _targets[target].adminDelay.withUpdate(newDelay, minSetback());
  ...
  function cancel(
    ...
    if (_schedules[operationId].timepoint == 0) {
      ...
      (bool isGuardian,) = hasRole(
        getRoleGuardian(getTargetFunctionRole(target, selector)), msgsender
      );
      if (!isAdmin && !isGuardian) {
        revert AccessManagerUnauthorizedCancel(
          msgsender, caller, target, selector
        );
      }
    }
  ...
  function _canCallSelf(
    ...
    (bool enabled, uint64 roleId, uint32 operationDelay) =
