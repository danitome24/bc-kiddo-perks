  ...
  IERC1155Receiver
{
  ...
  function proposalDeadline(
    ...
    return
      _proposals[proposalId].voteStart + _proposals[proposalId].voteDuration;
  }
  ...
  function propose(
    ...
    if (proposerVotes < votesThreshold) {
      revert GovernorInsufficientProposerVotes(
        proposer, proposerVotes, votesThreshold
      );
    }
  ...
  function _propose(
    ...
    if (
      ...
      revert GovernorInvalidProposalLength(
        targets.length, calldatas.length, values.length
      );
    }
    if (_proposals[proposalId].voteStart != 0) {
      revert GovernorUnexpectedProposalState(
        proposalId, state(proposalId), bytes32(0)
      );
    }
  ...
  function queue(
    ...
    _validateStateBitmap(
      proposalId, _encodeStateBitmap(ProposalState.Succeeded)
    );
  ...
  function castVoteBySig(
    ...
    bool valid = SignatureChecker.isValidSignatureNow(
      ...
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            BALLOT_TYPEHASH, proposalId, support, voter, _useNonce(voter)
          )
        )
      ),
      signature
  ...
  function _castVote(
    ...
    if (params.length == 0) {
      ...
      emit VoteCastWithParams(
        account, proposalId, support, weight, reason, params
      );
    }
  ...
  function _validateStateBitmap(
    ...
    if (_encodeStateBitmap(currentState) & allowedStates == bytes32(0)) {
      revert GovernorUnexpectedProposalState(
        proposalId, currentState, allowedStates
      );
    }
    return currentState;
  }
