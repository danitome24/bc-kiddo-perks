...
abstract contract GovernorTimelockAccess is Governor {
  ...
  mapping(address target => mapping(bytes4 selector => bool)) private
    _ignoreToggle;
  ...
  error GovernorMismatchedNonce(
    uint256 proposalId, uint256 expectedNonce, uint256 actualNonce
  );
  ...
  function propose(
    ...
    for (uint256 i = 0; i < targets.length; ++i) {
      ...
      (bool immediate, uint32 delay) = AuthorityUtils.canCallWithDelay(
        address(_manager), address(this), target, selector
      );
  ...
  function _queueOperations(
    ...
    for (uint256 i = 0; i < targets.length; ++i) {
      ...
      if (withDelay) {
        (, uint32 nonce) =
          _manager.schedule(targets[i], calldatas[i], etaSeconds);
  ...
  function _executeOperations(
    ...
    for (uint256 i = 0; i < targets.length; ++i) {
      ...
      if (controlled) {
        uint32 executedNonce =
          _manager.execute{ value: values[i] }(targets[i], calldatas[i]);
  ...
  function _cancel(
    ...
    if (etaSeconds != 0) {
      for (uint256 i = 0; i < targets.length; ++i) {
        ...
        if (withDelay) {
          bytes32 operationId =
            _manager.hashOperation(address(this), targets[i], calldatas[i]);
          ...
          if (nonce == _manager.getNonce(operationId)) {
            ...
            try _manager.cancel(address(this), targets[i], calldatas[i]) { }
              catch { }
          }
        }
      }
