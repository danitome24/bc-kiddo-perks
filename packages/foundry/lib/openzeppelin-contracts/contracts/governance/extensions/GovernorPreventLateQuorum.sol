...
abstract contract GovernorPreventLateQuorum is Governor {
  ...
  event LateQuorumVoteExtensionSet(
    uint64 oldVoteExtension, uint64 newVoteExtension
  );
  ...
  function proposalDeadline(
    ...
    return Math.max(
      super.proposalDeadline(proposalId), _extendedDeadlines[proposalId]
    );
  }
  ...
  function _castVote(
    ...
    uint256 result =
      super._castVote(proposalId, account, support, reason, params);
