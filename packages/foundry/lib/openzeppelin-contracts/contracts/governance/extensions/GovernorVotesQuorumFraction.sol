...
abstract contract GovernorVotesQuorumFraction is GovernorVotes {
  ...
  event QuorumNumeratorUpdated(
    uint256 oldQuorumNumerator, uint256 newQuorumNumerator
  );
  ...
  error GovernorInvalidQuorumFraction(
    uint256 quorumNumerator, uint256 quorumDenominator
  );
  ...
  function quorumNumerator(
    ...
    return
      _quorumNumeratorHistory.upperLookupRecent(SafeCast.toUint48(timepoint));
  }
  ...
  function _updateQuorumNumerator(
    ...
    _quorumNumeratorHistory.push(
      clock(), SafeCast.toUint208(newQuorumNumerator)
    );
