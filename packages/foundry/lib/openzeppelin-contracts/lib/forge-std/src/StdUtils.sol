...
abstract contract StdUtils {
  ...
  function bound(
    ...
    require(
      min <= max, "StdUtils bound(int256,int256,int256): Max is less than min."
    );
  ...
  function computeCreateAddress(
    ...
    return addressFromLast20Bytes(
      keccak256(
        abi.encodePacked(
          bytes1(0xda), bytes1(0x94), deployer, bytes1(0x84), uint32(nonce)
        )
      )
    );
  }
  ...
  function bytesToUint(
    ...
    require(
      b.length <= 32, "StdUtils bytesToUint(bytes): Bytes length exceeds 32."
    );
