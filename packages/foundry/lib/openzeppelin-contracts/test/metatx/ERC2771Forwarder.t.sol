...
import {
  ...
  CallReceiverMock
} from "@openzeppelin/contracts/mocks/CallReceiverMock.sol";
...
contract ERC2771ForwarderTest is Test {
  ...
  function setUp() public {
    ...
    _receiver =
      new CallReceiverMockTrustingForwarder(address(_erc2771Forwarder));
  ...
  function testExecuteBatchAvoidsETHStuck(
    ...
    for (uint256 i = 0; i < batchSize; ++i) {
      bytes memory data;
      bool succeed =
        uint256(keccak256(abi.encodePacked(initialBalance, i))) % 2 == 0;
    ...
    _erc2771Forwarder.executeBatch{ value: totalValue }(
      batchRequestDatas, refundReceiver
    );
