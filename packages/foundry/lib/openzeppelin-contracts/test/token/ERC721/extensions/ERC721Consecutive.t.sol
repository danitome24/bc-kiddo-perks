...
contract ERC721ConsecutiveTest is Test {
  function test_balance(
    ...
    ERC721ConsecutiveTarget token = new ERC721ConsecutiveTarget(
      toSingleton(receiver), batches, startingTokenId
    );
  ...
  function test_burn(
    ...
    uint256 tokenId =
      bound(unboundedTokenId, startingTokenId, startingTokenId + supply - 1);
    token.burn(tokenId);
  ...
  function test_transfer(
    ...
    ERC721ConsecutiveTarget token =
      new ERC721ConsecutiveTarget(receivers, batches, startingTokenId);

    uint256 tokenId0 = bound(unboundedTokenId[0], startingTokenId, batches[0]);
    uint256 tokenId1 =
      bound(unboundedTokenId[1], startingTokenId, batches[1]) + batches[0];
