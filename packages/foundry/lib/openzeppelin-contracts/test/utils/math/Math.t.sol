...
contract MathTest is Test {
  ...
  // Helpers
  function _asRounding(
    uint8 r
  ) private pure returns (Math.Rounding) {
  ...
  function _mulHighLow(
    ...
    uint256 carry = (
      (z1a & type(uint128).max) + (z1b & type(uint128).max) + (z0 >> 128)
    ) >> 128;
  ...
  function _addCarry(
    uint256 x,
    uint256 y
  ) private pure returns (uint256 res, uint256 carry) {
    unchecked {
      res = x + y;
    }
    carry = res < x ? 1 : 0;
